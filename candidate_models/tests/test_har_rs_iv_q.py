"""Smoke test for HAR-RS-IV-Q: synthetic 3-ticker x 500-day panel -> required columns, finite rv_hat."""
...
from candidate_models.har_rs_iv_q import HARRSIVQ
...
def test_needs_are_deduplicated_in_order():
    assert HARRSIVQ.name == "HAR-RS-IV-Q"
    needs = HARRSIVQ.needs
    assert needs == _EXPECTED_NEEDS, needs
    assert len(needs) == len(set(needs)), "needs must be deduplicated"


def test_har_rs_iv_q_predict_schema_and_finite():
    ...
    assert not pred.is_empty(), "HAR-RS-IV-Q produced no predictions on the synthetic panel"
